...
class ConfigManager:
    ...
    def load(self, file_path: Path) -> OperationResult[AppConfig]:
        ...
        for idx, c in enumerate(raw_connections):
            ...
            if protocol not in {"ftp", "ftpes", "ftps", "sftp"}:
                errors.append(
                    f"connections[{idx}] protocol must be one of 'ftp', 'ftpes', 'ftps', or 'sftp'."
                )
                continue
            ...
            try:
                profile = ConnectionProfile(
                    ...
                    ssh_key_path=str(c["sshKeyPath"]).strip() if c.get("sshKeyPath") else None,
                    ssh_known_hosts_policy=str(c.get("sshKnownHostsPolicy", "reject")).lower().strip(),
                )
            except Exception as ex:
