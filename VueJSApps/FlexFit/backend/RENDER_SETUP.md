# FlexFit Backend — Render Deployment Environment Variables
...
| `FRONTEND_URL` | `https://flex-fit-lkzh.onrender.com` | Tells the CORS middleware which origin to allow. If this is missing, **every preflight from the frontend is blocked** and login fails. |
...
| `FRONTEND_URL` missing | CORS blocks all requests from `https://flex-fit-lkzh.onrender.com`. Browser sees "Network Error" on login. |
...
```
🚀 FRONTEND_URL=https://flex-fit-lkzh.onrender.com
🌐 Allowed CORS origins: https://flex-fit-lkzh.onrender.com
...
```bash
curl -v https://dev-asterisks-github.onrender.com/api/debug/ping
curl -v --cookie "flexfit_session=TEST" https://dev-asterisks-github.onrender.com/api/session
```
