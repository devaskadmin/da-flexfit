...
// ─── PUT /session/:sessionId/save-day ── Save in-progress sets without completing
// Upserts workout_log + workout_log_sets for an in_progress session.
// Does NOT change session status, started_at, or completed_at.
router.put('/session/:sessionId/save-day', async (req, res) => {
  if (!req.session?.user?.id) {
    return res.status(401).json({ error: 'Unauthorized. Please log in.' });
  }
  ...
  const userId = req.session.user.id;
  const toSafe = (v) => { const n = Number(v); return Number.isFinite(n) ? n : 0; };

  const connection = await pool.getConnection();
  try {
    await connection.beginTransaction();

    // Verify session is in_progress and belongs to user
    const [[session]] = await connection.query(
      `SELECT id, user_id, status, workout_date, source_workout_schedule_id
       FROM workout_log_sessions WHERE id = ? LIMIT 1`,
      [sessionId]
    );
    if (!session) {
      await connection.rollback();
    ...
    if (session.status !== 'in_progress') {
      await connection.rollback();
      return res.status(409).json({ error: 'Can only save in-progress workouts.' });
    }

    const date             = session.workout_date;
    const sourceScheduleId = session.source_workout_schedule_id;

    for (const ex of exercises) {
      const sets = Array.isArray(ex.sets) ? ex.sets : [];

      // Resolve exerciseID
      let exerciseID = ex.exerciseId ? Number(ex.exerciseId) || null : null;
      ...
      const avgWeight = totalSets ? sets.reduce((a, s) => a + toSafe(s.weight), 0) / totalSets : 0;
      const avgReps   = totalSets ? sets.reduce((a, s) => a + toSafe(s.reps), 0) / totalSets : 0;
      const totalDur  = sets.reduce((a, s) => a + toSafe(s.duration), 0);
      const totalCals = sets.reduce((a, s) => a + toSafe(s.caloriesBurned), 0);
      const avgDist   = totalSets ? sets.reduce((a, s) => a + toSafe(s.distanceMiles), 0) / totalSets : 0;
      const avgSpeed  = totalSets ? sets.reduce((a, s) => a + toSafe(s.speedMph), 0) / totalSets : 0;

      // Find existing workout_log row for this session + exercise
      let workoutLogId = null;
      if (exerciseID) {
        const [existing] = await connection.query(
          `SELECT WorkoutLogID FROM workout_log
           WHERE workout_log_session_id = ? AND ExerciseID = ? AND UserID = ?
           LIMIT 1`,
          [sessionId, exerciseID, userId]
        );
        if (existing.length > 0) workoutLogId = existing[0].WorkoutLogID;
      }
      // Fallback: match by scheduleGroup label
      if (!workoutLogId && ex.scheduleGroup) {
        const [byGroup] = await connection.query(
          `SELECT WorkoutLogID FROM workout_log
           WHERE workout_log_session_id = ? AND source_schedule_group_label = ? AND UserID = ?
           LIMIT 1`,
          [sessionId, ex.scheduleGroup, userId]
        );
        if (byGroup.length > 0) workoutLogId = byGroup[0].WorkoutLogID;
      }

      if (workoutLogId) {
        // Update existing workout_log aggregates
        await connection.query(
          `UPDATE workout_log
           SET Weight = ?, Reps = ?, Sets = ?, Duration = ?, Calories = ?, Distance = ?, Speed = ?
           WHERE WorkoutLogID = ? AND UserID = ?`,
          [
            Math.round(avgWeight), Math.round(avgReps), totalSets,
            Math.round(totalDur),  Math.round(totalCals),
            Number(avgDist.toFixed(3)), Number(avgSpeed.toFixed(2)),
            workoutLogId, userId,
          ]
        );
      } else {
        // Insert new workout_log row for this in-progress session
        const [logResult] = await connection.query(
          `INSERT INTO workout_log
            (workout_log_session_id, UserID, ExerciseID,
             source_workout_schedule_id, source_schedule_group_label,
             WorkoutDate, performed_at, WorkoutType,
             Duration, Reps, Sets, Weight)
           VALUES (?, ?, ?, ?, ?, ?, NOW(), ?, ?, ?, ?, ?)`,
          [
            sessionId, userId, exerciseID || null,
            sourceScheduleId,
            sanitizeText(ex.scheduleGroup || '', 120) || null,
            date,
            sanitizeText(ex.workoutType || 'Strength'),
            Math.round(totalDur), Math.round(avgReps), totalSets, Math.round(avgWeight),
          ]
        );
      ...
      for (let i = 0; i < sets.length; i++) {
        ...
        await connection.query(
          `INSERT INTO workout_log_sets
            (workout_log_id, set_number, reps, weight, duration_minutes,
             calories_burned, distance_miles, speed_mph, completed)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
           ON DUPLICATE KEY UPDATE
             reps             = VALUES(reps),
             weight           = VALUES(weight),
          ...
          [
            workoutLogId, i + 1,
            toSafe(s.reps),  toSafe(s.weight), toSafe(s.duration),
            toSafe(s.caloriesBurned), toSafe(s.distanceMiles), toSafe(s.speedMph),
            s.done ? 1 : 0,
          ]
        );
      }
    ...
    await connection.commit();
    return res.status(200).json({ message: 'Day saved successfully.', sessionId });
  } catch (err) {
